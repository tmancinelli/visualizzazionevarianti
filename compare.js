class LaCamera {
  ...
  constructor() {
    ...
    this.#CETEIcean.addBehaviors({
      "tei": {
        ...
        "l": e => {
          const span = document.createElement('span');
          if (!e.getElementsByTagName("tei-app").length) {
            span.setAttribute('style', 'filter: blur(1.5px)');
          }
          span.innerHTML = e.innerHTML;
          return span;
        }
  ...
  async initialize() {
    const txt = await fetch("data/lacamera_varianti_full.xml").then(r => r.text());
  ...
  createTimeline() {
    ...
    this.#selectWitnesses.addEventListener("change", () => timeline.setSelection(this.#selectWitnesses.value));
  }
